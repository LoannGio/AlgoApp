...
<<<<<<< HEAD
			for(Set<V> D : SubsetCreator.allSubsetsOfSizeN(dominating, i)) {
				if(collisions) {
					//Si l'ensemble n'est pas indépendant on ne le considère pas
...
>>>>>>> ed67b17380d4d3688538b228b2f972f0f6441f78
						continue;
				} else {
					if (dominates(G, dominated, D))
						return D;
...
>>>>>>> ed67b17380d4d3688538b228b2f972f0f6441f78
	 ...
	 * (null si la mï¿½thode gloutonne ne trouve pas de sous-ensemble de taille
	 * <= 6
	 */
	...
	/*
	 * Retourne un ensemble dominant du graphe G qui domine un sous-ensemble de
	 * sommets dominated d'un graphe (null si la mï¿½thode gloutonne ne trouve
	...
	/*
	 * Retourne un ensemble dominant du graphe G (null si la mï¿½thode gloutonne
	...
	public static <V, E> Set<V> dominatingSetGreedyGoal(SimpleGraph<V, E> G, Set<V> dominatingGoal, Set<V> dominating,
			Set<V> dominated) {
		Set<V> dominatedCopy = new HashSet<V>(dominated);
		Set<V> dominatingCopy = new HashSet<V>(dominating);
<<<<<<< HEAD
